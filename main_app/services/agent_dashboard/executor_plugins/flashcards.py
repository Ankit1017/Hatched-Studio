...
def _build_executor(context: AssetExecutorPluginContext) -> AssetExecutor:
    def _execute(payload: IntentPayload, settings: GroqSettings) -> AgentAssetResult:
        ...
        result = context.flashcards_service.generate(
            topic=topic,
            card_count=int(payload.get("card_count", 20)),
        ...
        return build_parsed_asset_result(
            ...
            parsed_content=result.parsed_flashcards,
            parse_error=result.parse_error,
            raw_text=result.raw_text,
            parse_note=result.parse_note,
            cache_hit=result.cache_hit,
            failure_message="Flashcards generation failed.",
        )

    return _execute


PLUGIN = AssetExecutorPlugin(intent="flashcards", build_executor=_build_executor)
